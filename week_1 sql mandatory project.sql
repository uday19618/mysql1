...
-- Execute the following queries on the employee database
...
select first_name,last_name,concat(salary*1.15,'$') as salary from employees;

--  4)Write a SQL query to list the employee's name and job name as the format of "Employee & Job".

select concat(first_name,job_id) as 'employee & job' from employees;


-- 5) Write a SQL query to find those employees with the hire date in a format like February 22, 1991. Return employee ID, employee name, salary, and hire date.


select employee_id,concat(first_name,last_name) as 'name',salary,date_format(hire_date,'%m %d %y') from employees;
...
select char_length(first_name) + char_length(last_name) as full_name_char_count, (concat(first_name, " ", last_name)) as full_name  from employees;
...
select * from employees where format(hire_date,'%y %m %d') <1991;

-- 10) Write a SQL query to compute the average salary of those employees who work as ‘shipping clerk’. Return average salary.

select avg(salary) as average_salary from employees where job_id='sh_clerk' ;
